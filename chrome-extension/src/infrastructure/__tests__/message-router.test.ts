...
import { createMessageRouter, type MessageRouterOptions } from "../message-router.js";
...
describe("createMessageRouter — message parsing and validation", () => {
  test("rejects invalid JSON with UNKNOWN_ACTION", async () => {
    ...
    const handleMessage = createMessageRouter(createRouterOptions({ wsClient }));
  ...
  test("accepts all valid action names", async () => {
    ...
    for (const action of validActions) {
      const wsClient = createMockWsClient();
      const handleNavigate = vi.fn().mockResolvedValue({ id: "req-1", result: "ok" });
      const handleScreenshot = vi.fn().mockResolvedValue({ id: "req-1", result: "ok" });
      const handleListTabs = vi.fn().mockResolvedValue({ id: "req-1", result: { tabs: [] } });
      const handleCloseTab = vi.fn().mockResolvedValue({ id: "req-1", result: { closed: true } });
...
describe("createMessageRouter — disabled bridge", () => {
  ...
  test("logs info message when bridge is disabled", async () => {
    const wsClient = createMockWsClient();
    const options = createRouterOptions({ wsClient, enabled: { current: false } });
...
describe("createMessageRouter — navigate routing", () => {
  test("routes navigate actions to handleNavigate handler", async () => {
    ...
    const handleMessage = createMessageRouter(createRouterOptions({
      wsClient,
      handleNavigate,
    }));
...
describe("createMessageRouter — listTabs routing", () => {
  test("routes listTabs actions to handleListTabs handler", async () => {
    ...
    const handleListTabs = vi.fn().mockResolvedValue({
      id: "req-1",
      result: { tabs: [{ tabId: 1, url: "https://example.com", title: "Example", active: true }] },
    });
...
describe("createMessageRouter — closeTab routing", () => {
  ...
  test("returns TAB_NOT_FOUND when tab does not exist", async () => {
    ...
    await handleMessage(
      JSON.stringify({ id: "req-1", action: "closeTab", params: { tabId: 999 } }),
    );
...
describe("createMessageRouter — logging", () => {
  ...
  test("truncates long messages in logs", async () => {
    ...
    const logCall = (options.logger.info as ReturnType<typeof vi.fn>).mock.calls.find(
