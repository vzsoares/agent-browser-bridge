...
import { dispatch, ALL_ACTIONS } from "../dispatcher.js";
...
describe("Message dispatcher", () => {
  ...
  describe("routing all 8 action types", () => {
    ...
    test("routes 'click' action", async () => {
      const result = await dispatch("click", { selector: "#nonexistent", timeout: 10 });
      expect(result).toBeDefined();
      const r = result as Record<string, unknown>;
    ...
    test("routes 'type' action", async () => {
      const result = await dispatch("type", { selector: "#nonexistent", text: "hello", timeout: 10 });
      const r = result as Record<string, unknown>;
    ...
    test("routes 'waitForElement' action", async () => {
      const result = await dispatch("waitForElement", { selector: "div", timeout: 10 });
      const r = result as Record<string, unknown>;
    ...
    test("routes 'waitForText' action", async () => {
      const result = await dispatch("waitForText", { text: "nonexistent", timeout: 10 });
      const r = result as Record<string, unknown>;
      expect(r).toHaveProperty("found");
      expect(r.found).toBe(false);
    });
